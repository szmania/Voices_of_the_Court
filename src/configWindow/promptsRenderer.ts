    ...
    try {
        ...
        const defaultPrompts: any = {
            en: {
                ...
                diaryPrompt: "Message for the POV Character: This is the main narrative. The response should be natural in length. Do not artificially stretch the text. A concise, detail-rich response is preferable. A maximum of 1500 characters is a limit, not a goal. Use italics for descriptions. Format direct speech like this: - Speech, - POV's action/thought. Every detail must serve a purpose. Describe only what is important right now: the key action, 1-2 vivid sensations or objects that reveal character or atmosphere. Avoid generic phrases and bad clichés."
            },
            zh: {
                ...
                diaryPrompt: "给POV角色的信息：这是主要叙述。回应应该是自然的长度。不要人为地拉长文本。简洁而细节丰富的回应更为可取。最多1500个字符是一个限制，而不是目标。使用斜体进行描述。直接引语的格式如下：- 话语，- POV的动作/想法。每个细节都必须有目的。只描述现在重要的事情：关键动作，1-2个生动的感觉或揭示角色或氛围的物体。避免陈词滥调和糟糕的俗套。"
            },
            ru: {
                ...
                diaryPrompt: "Сообщение для персонажа от первого лица: Это основной нарратив. Ответ должен быть естественной длины. Не искусственно растягивайте текст. Предпочтительнее краткий, насыщенный деталями ответ. Максимум 1500 символов - это предел, а не цель. Используйте курсив для описаний. Формат прямой речи следующий: - Речь, - действие/мысль от первого лица. Каждая деталь должна служить цели. Описывайте только то, что важно прямо сейчас: ключевое действие, 1-2 ярких ощущения или предмета, раскрывающих характер или атмосферу. Избегайте общих фраз и плохих клише."
            },
            fr: {
                ...
                diaryPrompt: "Message pour le personnage au point de vue : Ceci est le récit principal. La réponse doit être de longueur naturelle. N'étirez pas artificiellement le texte. Une réponse concise et riche en détails est préférable. Un maximum de 1500 caractères est une limite, pas un objectif. Utilisez l'italique pour les descriptions. Formatez le discours direct comme ceci : - Parole, - action/pensée du point de vue. Chaque détail doit avoir un but. Décrivez uniquement ce qui est important maintenant : l'action clé, 1 à 2 sensations ou objets vifs qui révèlent le caractère ou l'atmosphère. Évitez les phrases génériques et les mauvais clichés."
            }
        };

---
title: README_japanese
type: note
permalink: votc/readme-japanese
---
...
**Windows**
1. 最新リリースから `.exe` インストーラーをダウンロードします。
2. インストーラーを実行し、画面の指示に従います。
3. VOTC-CE アプリケーションを起動します。

**macOS**
1. 最新リリースから `.zip` または `.dmg` ファイルをダウンロードします。
2. `.dmg` をダウンロードした場合、それを開き、アプリケーションアイコンを `/Applications` フォルダにドラッグします。
3. `.zip` をダウンロードした場合、それを解凍し、アプリケーションアイコンを `/Applications` フォルダにドラッグします。
4. **重要な最初のステップ**: アプリを実行するには、AppleのGatekeeperセキュリティをバイパスする必要があります。アプリケーションアイコンを右クリック（またはCtrlキーを押しながらクリック）し、メニューから **開く** を選択します。
5. 警告ダイアログが表示されます。**開く** ボタンをクリックして確認します。この操作はアプリを初めて実行するときにのみ必要です。

**CK3 Modのセットアップ**
1. リリースページからVOTC-CE modファイルの最新バージョン（通常は `.zip` ファイル）をダウンロードします。
2. 内容をCK3 modフォルダ（通常は `Documents/Paradox Interactive/Crusader Kings III/mod` にあります）に解凍します。
3. Crusader Kings IIIを起動し、ランチャーで「Voices of the Court」modを有効にします。
